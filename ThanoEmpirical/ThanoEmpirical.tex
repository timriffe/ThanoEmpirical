...
bound belongs to the completed lifespan of 96. While there are some observations
...
deaths occur, and because the HRS provides the best density and spread of data
points over this window. This is why the study area has a lower
chronological age bound of 72 and an upper completed lifespan of 96.
...
measures, and health behaviors. In all, we summarize results from 82
...
from 0 to 1, respectively. This choice is far from clean, but it
is good enough to meet the goals of this study. Other variables
were coded in a similarly ``good-enough'' fashion. \footnote{The pre-processing
of variables is full of details that would clutter this paper. Rather than a
lengthy and detailed appendix describing the case by case treatment of
variables, we make the full code base used in the generation of results available in a github repository.}
...
visits were not rescaled. These choices are simply to simplify the
visual interpretation of surfaces, and they do not drive the quantitative summary measures we use later.
...
composition bias. To overcome birth cohort heterogeneity within the surface,
...
We specify a loess model of the given characteristic over chronological age, thanatological age, and quinquennial birth cohorts, using all observations of since-deceased individuals from the 1900 through the 1929 birth cohorts. We fit the model using the \texttt{loess()} function in base \texttt{R} \citep{cleveland1992local,Rcore2013}\footnote{Using the fitted model, surfaces are produced using the related loess prediction function, \texttt{predict.loess()}. The smoothing
parameter, \texttt{spar}, is set to 0.7 for the results we present in the paper.
